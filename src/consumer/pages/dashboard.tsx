  ...
  const loadDashboardData = React.useCallback(async () => {
    ...
    try {
      ...
      const verifiedCredentials = credentials.filter(cred =>
        (cred as any).proof && (cred as any).proof.length > 0
      ).length;
      ...
      setRecentCredentials([] as VerifiableCredential[]);
                ...
                {recentCredentials.length > 0 ? (
                  <div className="space-y-4">
                    {recentCredentials.map((credential, index) => (
                      <div key={(credential as any).id || index} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-medium">
                              {(credential as any).type?.join(', ') || 'Credential'}
                            </p>
                            <p className="text-sm text-gray-600">
                              Issued by {typeof (credential as any).issuer === 'string' ? (credential as any).issuer : (credential as any).issuer?.name || 'Unknown'}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          {(credential as any).proof ? (
