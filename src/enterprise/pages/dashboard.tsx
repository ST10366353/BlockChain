  ...
  const loadEnterpriseData = React.useCallback(async () => {
    ...
    try {
      ...
      const trustedPartners = trustResult.status === 'fulfilled'
        ? (trustResult.value as any[]).filter((partner: any) => partner.status === 'active')
        : [];

      // Process audit logs
      const auditLogs = (auditResult.status === 'fulfilled'
        ? (auditResult.value as any[])
        : []).map((log: any) => ({
          id: log.id ?? `${log.timestamp}-${log.type}`,
          type: log.type ?? 'event',
          timestamp: log.timestamp ?? Date.now(),
          description: log.description ?? '',
          level: log.level ?? (log.success === false ? 'error' : 'info'),
              ...
              <CardContent>
                {recentActivity.length > 0 ? (
                  <div className="space-y-4">
                    {recentActivity.map((activity: {id: string; type: string; timestamp: number; description: string; level?: string; action?: string; resource?: string}, index: number) => (
