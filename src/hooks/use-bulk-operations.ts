...
export function useBulkOperations() {
  ...
  const updateProgress = React.useCallback((updates: Partial<BulkOperationProgress>) => {
    setProgress(prev => {
  ...
  const bulkVerifyCredentials = React.useCallback(async (
  ...
  const bulkRevokeCredentials = React.useCallback(async (
    credentials: CredentialSummary[],
    options: BulkOperationOptions = {}
  ): Promise<BulkOperationResult<CredentialSummary>[]> => {
    const {
  ...
  const bulkUpdateConnections = React.useCallback(async (
  ...
  const bulkExportCredentials = React.useCallback(async (
    credentials: CredentialSummary[],
...
export function useBulkSelection<T extends { id: string }>(items: T[] = []) {
  ...
  const isSelected = React.useCallback((id: string) => selectedItems.has(id), [selectedItems])
